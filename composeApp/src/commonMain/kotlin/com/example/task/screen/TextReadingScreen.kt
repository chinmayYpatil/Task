...
import com.example.task.viewmodel.RecordingState
...
fun TextReadingScreen(mainViewModel: MainViewModel) {
    ...
    Column(
        ...
        Box(
            ...
            if (uiState.recordingState == RecordingState.LOADING_TEXT) {
        ...
        when (uiState.recordingState) {
            RecordingState.READY_TO_RECORD, RecordingState.RECORDING -> {
                RecordButton(
        ...
        if (uiState.recordingState == RecordingState.REVIEW) {
            Button(
...
private fun RecordButton(
    ...
    IconButton(
        ...
        enabled = state == RecordingState.READY_TO_RECORD || state == RecordingState.RECORDING,
        modifier = Modifier
            .size(100.dp)
    ...
    val statusText = when (state) {
        RecordingState.READY_TO_RECORD -> "Tap to Start Recording"
        RecordingState.RECORDING -> "${elapsedTime}s / ${MAX_DURATION_SEC}s (Tap to stop)"
        else -> ""
    }
